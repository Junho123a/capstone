...
class TrainingCallback:
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.socketio.emit('training_metrics', metrics_data)
    ...
    def _update_loss_chart(self):
        ...
        self.socketio.emit('loss_chart', {'chart': f'data:image/png;base64,{plot_url}'})
    ...
    def _update_accuracy_chart(self):
        ...
        self.socketio.emit('accuracy_chart', {'chart': f'data:image/png;base64,{plot_url}'})
...
def analyze_sentiment(company):
    ...
    try:
        ...
        if not sentiment_df.empty:
            ...
            return jsonify({"error": "감성 분석을 위한 뉴스 데이터가 충분하지 않습니다."})
...
def get_stock(ticker):
    ...
    try:
        ...
        if stock_data.empty:
            return jsonify({"error": "주가 데이터를 가져오는데 실패했습니다."})
    ...
    try:
        ...
        for feature in features:
            if feature not in merged_data.columns:
                return jsonify({"error": f"데이터에 {feature} 열이 없습니다."})
        ...
        if len(X) == 0:
            return jsonify({"error": "시퀀스 데이터를 생성할 수 없습니다. 데이터가 충분하지 않습니다."})
